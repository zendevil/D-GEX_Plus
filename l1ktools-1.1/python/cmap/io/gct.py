    ...
    def write(self, ofile, mode = 'gctx'):
        ...
        if mode == 'gctx':
            ...
            with warnings.catch_warnings():
                ...
                # store the matrix
                h5f.create_group('/', '0')
                h5f.create_group('/0/DATA', '0', createparents = True)
                h5f.create_array('/0/DATA/0', 'matrix', self.matrix.transpose().astype(numpy.float32))
                # store the column annotations, except for "ind"; held internally only
                h5f.create_group('/0/META', 'COL', createparents = True)
                for field in [x for x in self.get_chd() if x != 'ind']:
                    h5f.create_array('/0/META/COL', field,
                                    numpy.array(self.get_column_meta(field)))
                # now the row annotations for "ind"
                h5f.create_group('/0/META', 'ROW', createparents = True)
                for field in [x for x in self.get_rhd() if x!= 'ind']:
                    h5f.create_array('/0/META/ROW', field,
